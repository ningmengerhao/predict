...
def init_route(n_route, n_cities):
    ...
    for i in range(n_route):
        routes[i] = np.random.choice(
            range(n_cities), size=n_cities, replace=False)
    return routes
...
def mutation(routes, n_cities):
    ...
    for i in range(len(routes)):
        if p_rand[i] < prob:
            mut_position = np.random.choice(
                range(n_cities), size=2, replace=False)
...
if __name__ == '__main__':
    ...
    fitness_values = get_all_routes_fitness_value(
        routes, dist_matrix)  # 计算所有初始路线的适应度
    ...
    for i in range(epoch):
        ...
        if fitness_values[best_route_index] > best_fitness:
            ...
            not_improve_time += 1
        if (i+1) % 200 == 0:
            print('epoch: {}, 当前最优路线距离： {}'.format(
                i+1, 1/get_route_fitness_value(best_route, dist_matrix)))
