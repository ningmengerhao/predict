...
def get_table(province_name):
    ...
    while control != "上一页":
        ...
        source = driver.find_element(
            By.XPATH, '//*[@id="cjsj_table"]/table/tbody').find_elements(By.XPATH, '//td')
        for x in source:
            price_table.append(x.get_attribute(
                'innerText').replace("↑", "").replace("↓", "-"))
        # 判断当前是否仍然需要翻页
        try:
            page = driver.find_element(
                By.XPATH, '//*[@id="cjsj_table_pager"]/div[1]/a[@class="active"]').get_attribute('data-page')
            print(province_name, page)
            control = driver.find_element(By.LINK_TEXT, '下一页')
            ActionChains(driver).move_to_element(
                control).click(control).perform()
            control = "下一页"
        except:
            control = "上一页"
    ...
    price_table.to_csv("./data/%s.csv" % province_name,
...
def get_province_urls(base_url="http://data.eastmoney.com/cjsj/oil_city.aspx?city=%E5%8C%97%E4%BA%AC"):
    ...
    options = driver.find_element(
        By.ID, 'city-select').find_elements(By.XPATH, '//option')
